...
class Volunteer_Management_And_Tracking_Common {
	...
	public function get_manage_volunteers( $args=array() ) {
        ...
        $sql .= '(SELECT uumfn.id as user_id, ';
        $sql .=         'uumfn.user_email, ';
        $sql .= 'TRIM(IF((STRCMP(uumfn.first_name, "") = 0 OR uumfn.first_name IS NULL)  AND ';
        $sql .=     '((STRCMP(umln.last_name, "") = 0 OR umln.last_name IS NULL)) , ';
        $sql .=     'uumfn.display_name, ';
        $sql .=     'CONCAT(uumfn.first_name, " ", umln.last_name))) as display_name FROM ';
        $sql .= '(SELECT u.id, ';
        $sql .=         'u.display_name, ';
        $sql .=         'u.user_email, ';
        $sql .=         'umfn.first_name FROM ';
        $sql .= '(SELECT id, ';
        $sql .=          'user_email, ';
        $sql .=          'display_name FROM ';
        $sql .= $prefix . 'users WHERE id in (' . $volunteer_ids .'))as u ';
        $sql .= 'LEFT JOIN ';
        $sql .= '(SELECT user_id, ';
        $sql .= 'meta_value as first_name FROM ';
        $sql .= $prefix . 'usermeta WHERE meta_key="first_name") AS umfn ';
        $sql .= 'ON u.id=umfn.user_id) as uumfn ';
        $sql .= 'LEFT JOIN ';
        $sql .= '(SELECT user_id, ';
        $sql .= '        meta_value as last_name FROM ';
        $sql .= $prefix . 'usermeta WHERE meta_key="last_name") AS umln ';
        ...
        $sql .= 'ON p.post_author=dn.user_id OR p.post_author IS NULL) AS vh ';
        $sql .= 'LEFT JOIN ';
